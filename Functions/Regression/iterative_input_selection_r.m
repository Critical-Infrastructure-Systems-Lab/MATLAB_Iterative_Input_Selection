...
function [result] = iterative_input_selection_r(subset,M,nmin,ns,p,epsilon,max_iter,Vflag)

% This function implements the IIS algorithm for regression problems
%
% subset   = observations
...
while (diff > epsilon) && (iter <= max_iter)
    ...
    disp('Ranking:');
    k = size(input,2);
    [ranking] = input_ranking_r(matrix_ranking,M,k,nmin);     
    ...
    for i = 1:p
        if f == 1
        [siso_model] = crossvalidation_extra_tree_ensemble_r([subset(:,features(i)) rank_output],M,1,nmin,ns,0);        
        else
	...
	k = length(miso_input);	
    if f==1
	[miso_model] = crossvalidation_extra_tree_ensemble_r([subset(:,miso_input) miso_output],M,k,nmin,ns,1);				 
    else
